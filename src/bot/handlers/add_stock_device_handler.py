...
from src.db_app import ApiTempJS, TempJS
...
async def add_stock_device(message: Message, state: FSMContext):
    ...
    if bot_api_db.check_device_id(device_name):
        ...
        await message.answer(text="Данные складского прибора сохранены в бд")

    elif os.path.exists("temp_device.json"):
        d_js = ApiTempJS("temp_device.json")
        device = d_js.read()
        device.update(stock_device_data)
        bot_api_db.save_stock_device_into_db_from_bot(device)
        await message.answer(text="Данные складского прибора сохранены в бд")

    else:
        tepm_js = ApiTempJS("temp_stock_device.json")
        tepm_js.write(stock_device_data)
...
async def add_device_name(message: Message, state: FSMContext):
    tepm_js = ApiTempJS("temp_stock_device.json")
    device_name = tepm_js.read()["device_name"]
...
async def add_company_name_for_device(message: Message, state: FSMContext):
    ...
    if bot_api_db.check_company_id(company_name):
        ...
        api_js = ApiTempJS("temp_device.json")
        api_js.write(device_data)
...
async def add_device(message: Message, state: FSMContext):
    ...
    if bot_api_db.check_type_id(type_title):
        ...
        await message.answer(text="Данные устройства добавлены в бд")
    else:
        device_data = await state.get_data()
        api_js = ApiTempJS("temp_device.json")
        api_js.write(device_data)
...
async def add_company_name(message: Message, state: FSMContext):
    temp_js = ApiTempJS("temp_device.json")
    company_name = temp_js.read()["company_name"]
    await state.set_data({"company_name": company_name})
...
async def add_type_title(message: Message, state: FSMContext):
    temp_js = ApiTempJS("temp_device.json")
    type_title = temp_js.read()["type_title"]
    await state.set_data({"type_title": type_title})
